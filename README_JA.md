...
- 📑 **セッション & バッファ自動保持 & タブのドラッグ並び替え**: 保存していない書き殴りのメモも次回起動時に完全復元。ポインターイベントによる軽快かつ確実なタブのドラッグ＆ドロップ並び替えに対応。
- 🗂️ **フォルダ一括読み込み & ファイルドロップ**: 大規模なワークスペースフォルダも非同期スキャンでフリーズせず一括読込。外部ファイルの直接ドロップや、保存時の見出し自動推論ファイル名にも対応。
- 🔤 **スマート文字コード判別**: UTF-8 および Shift_JIS（CP932）を自動検出・保持。
- 📄 **装飾なしテキスト (.txt) エクスポート**: Markdown記号（`#`, `*`, `[]()`等）をワンクリックで除去して平文保存。
- ⌨️ **ショートカット自由変更 & ウィンドウ制御**: 設定画面から全ショートカットのキー割り当てを自由に変更可能（初期値リセット対応）。F11での最大化/全画面トグル、Zen集中モード (<kbd>Ctrl+Shift+Z</kbd>)、どこからでも画面復帰できる常駐ショートカット (<kbd>Ctrl+Alt+M</kbd>) を完備。
...
> [!TIP]
> アプリ内の全ショートカットキーは **設定（`⚙️`）> ショートカット** タブから自由にお好みのキーへ変更できます！ 右クリックのコマンドメニューやツールチップのショートカット表示も自動的に連動します。
