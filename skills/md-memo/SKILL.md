...
- The built-in agent definitions changed after v1.8.0 (`claude -p`, `codex exec`, `agy` without `--dangerously-skip-permissions`; not run on the maintainer's machine); an agents file written earlier keeps its old copies, which Settings -> Agent lists (interfaces.md 3.1.3). Check `--help` before relying on any flag. An agent with a permission-skipping flag, or a shell agent that would run the appended instruction as code, is confirmed by the user once before it runs.
