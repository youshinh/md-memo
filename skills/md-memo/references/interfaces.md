...
- Ranges inside fenced code blocks (backtick or tilde fences), inline code, Markdown links `[t](u)`, bare `http(s)://` URLs and HTML comments (3.1.3) are never slots. A slot that starts or ends inside one of those ranges is skipped (a slot that holds a whole comment is not: the comment stays part of its text).
...
#### 3.1.3 HTML comments: hidden text that never runs
...
| commentToggle (label "Toggle Comment"; group Line Operations; 3.1.3; ignored while an IME composes) | Ctrl+/ | Cmd+/ |
...
Substring filter (case-insensitive) over each entry's title and description; Up/Down/Enter/Esc. Entries: New Tab, Open File, Open Folder, "Ask AI", "Command Bar" (opens in the mode used last), "Command Bar: Run a Command" (manual CLI mode), "Command Bar: AI Writes the Command" (AI mode), "Insert task snippet" (ja 「タスクのひな形を挿入」: the snippet list of 3.1.2 at the caret), Mobile Drop, Voice Input, "Quick Capture" (Windows only: it exists only where the backend offers `openQuickCapture`; ja クイックキャプチャ), "Open inbox folder" (ja 「受信フォルダを開く」; only while `inbox.enabled` is true, both platforms), three prompt presets (Polish, Bullet points, Action items: they open the Ask AI bar with the instruction pre-filled), "Diagram: Convert Selection to Mermaid", "Diagram: Generate Image with Gemini", "Diagram: Generate Image Prompt from Mermaid", AI proofread, "Toggle comment" (ja 「コメントの切り替え」, Ctrl+/, 3.1.3), Export plain text, Zen mode, Full screen, Toggle split (22 commands on macOS with the hot folder off; Windows adds Quick Capture, and the hot folder adds Open inbox folder); plus every note found in the open workspace folder (title, relative path and first line; Enter opens it in a new tab). It has no settings entries.
