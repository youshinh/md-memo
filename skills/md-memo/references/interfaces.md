...
| Turn an image into text in today's scrap | `md-memo ocr <image>` (section 1.5). It may send the image to the cloud vision model and it writes to the user's scrap, so do not run it on the user's files without saying so | no |
| Learn where things are (config and scrap folders, today's scrap file, whether the app runs) | `md-memo info` (section 1.6): read-only, prints no secret | no |
| Find or list the user's scraps | `md-memo scrap path\|list\|search` (section 1.7): read-only; a search hit carries its nearest heading | no |
| See the settings | `md-memo config get [key.path]` (section 1.8): `config.json` with every key, token and password hidden. This, not the file, is what an agent may read | no |
| Put this skill where an agent reads it (also after a Homebrew install, whose cask has no `skills/` folder) | `md-memo agent install-skill [--claude\|--codex\|--dir <path>]` (section 1.9): copies the skill built into the program, with a version marker; only when the user asked, builds newer than 1.8.0 | no |
| Save the open note to a file with non-ASCII (Japanese) text intact | `md-memo buffer get --out <file>` (section 1.2): the CLI writes UTF-8 itself, avoiding the shell's re-encoding | yes |
| Have images and audio in a folder turned into notes | the hot folder (`inbox.*`, section 4.10): the user switches it on in Settings -> Sync. It MOVES the files it processes and may send them to a cloud model, so never drop or place files into it without the user's consent | yes (the user's running app does the work) |
| Jot a note without opening the app (Windows) | the Quick Capture popup (section 4.10); only the user opens it | yes (the user drives it) |
...
Entry point: `main.go` (`main`) and, on Windows, `cmd/md-memo-cli` (the console-subsystem twin, see the first row of 1.0): both call `cli.Main` (`pkg/cli/entry.go`), which runs everything that does not start the GUI. `pkg/cli/registry.go` is the one list of command words and whether each needs the running app, `pkg/cli/client.go`, `pkg/cli/headless.go`, `pkg/cli/help.go`, `pkg/cli/format.go`; the newer commands are in `pkg/cli/info.go`, `scrapcmd.go`, `configcmd.go`, `bufferout.go`, with the shared read-only reader of `config.json` in `pkg/cli/config.go`.

### 1.0 How the binary is dispatched

| Invocation | Effect | Notes |
|---|---|---|
| `md-memo-cli <same arguments>` (Windows: `md-memo-cli.exe` next to `md-memo.exe` in the zip of the release that contains it; built from `cmd/md-memo-cli`, version from `-X main.version`, `dev` in a plain build) | Every command of this section, through the same `cli.Main` as `md-memo`, but a console-subsystem program: PowerShell, cmd and CI wait for it and get its real exit code and output (`md-memo.exe` is built `-H windowsgui`, so they may not: `troubleshooting.md` section 1). It NEVER starts the GUI. With no arguments, a file path or piped text it sends the same legacy message as `md-memo` (`activate`, `open` with an absolute path, `pipe` with the text, title words as heading; a file argument wins over stdin) to the running app found through `ipc-session.json`, and prints nothing; when there is none: stderr `Error: md-memo is not running. Start md-memo.exe first.`, exit 1 (an unknown word that is not an existing file: `Error: "..." is neither a command nor an existing file`, exit 1) | Use it in scripts, agents and CI on Windows when it exists. Not shipped for macOS, where the app binary already waits in a terminal. `md-memo-cli --version` prints `md-memo <version>`. |
| `md-memo buffer ...`, `md-memo tab ...`, `md-memo ui ...` | JSON-RPC client against the running GUI | Needs `<cfg>/ipc-session.json` and a live process. Otherwise: stderr `Error: md-memo is not running. Start MD-Memo first: buffer, tab and ui need the running app (jev, agent, ocr, info, scrap and config do not).`, exit 1 (the list of names is generated from the registry; older builds read `jev, agent and ocr`; before 1.7.1 the message ended `Launch md-memo first or use --headless.`, which was wrong: `--headless` does not support these three). |
| `md-memo jev ...`, `md-memo agent ...`, `md-memo ocr <image>`, `md-memo info`, `md-memo scrap ...`, `md-memo config get ...` | Local computation, no GUI | Also reachable as `md-memo --headless jev ...`. `ocr` (section 1.5) needs no running instance either: the Explorer Send To entry runs it. `info`, `scrap` and `config get` (sections 1.6 to 1.8) only READ files and create nothing. Builds from before they existed (1.8.0 and older) do not know these words: there `md-memo info` is not a command (a bare word starts the GUI or is taken as a file name), so check `md-memo --version` / `md-memo --help` first. |
| `md-memo --help`, `-h`, `help`, `help <command>`, `help pipe`, `help rpc`, `<command> --help`, `--version`, `-v` | Prints the usage of every command (or of one topic), or the version, on stdout and exits 0. The top-level text also carries the pipe and JSON-RPC reference (session file, wire format, methods, error codes); `main_help_test.go` fails when a method of `app_rpc.go` is missing from it. Any other word followed by `-h`/`--help` (say `md-memo share --help`) prints the top-level usage too | Handled first in `main` (`cli.HelpRequest`, `pkg/cli/help.go`), so nothing starts or raises the GUI: run this before guessing flags. Only LEADING flags count, so `buffer append hello -h` still appends `hello -h`, and `jev verify` is never intercepted past its action word (`jev verify -h` reaches the flag parser and exits 1, fail closed: its exit 0 means "safe"). Older builds (before 1.7.1) have no top-level help: `--help` started the GUI there. |
| `md-memo --headless help` (also `--help`, `-h`) | Prints the headless usage (jev, agent, ocr, info, scrap and config) | The same commands as without `--headless`; it lists only the ones that run standalone. |
...
| Finder "Open With", a double-click on a `.md` / `.txt` file, a drop on the Dock icon (macOS only) | AppKit delivers an Apple Event, not an argument: the delegate's `application:openFiles:` (`window_darwin.go`) calls `App.OpenFromOS` (`app_openfiles.go`). A running app opens the file in a new tab and fronts the window; a cold start queues it and `GetStartupFile` hands it to the page as its first tab | Builds up to 1.7.1 had no such handler, so macOS answered "MD-Memo cannot open files in the "Markdown Document" format". Not verifiable on Windows; the Apple Event path needs a Mac. |
...
- Flag parsing uses Go's `flag` package: `-x` and `--x` are equivalent; flags must come BEFORE positional text; parsing stops at the first non-flag. Put `--` before content that starts with `-`. (`buffer set/append/replace/replace-selection` additionally auto-insert `--` before the first unknown flag-like word, e.g. Markdown `- [ ] item`; `jev verify` does not.) The exception is `info`, `scrap` and `config get` (sections 1.6 to 1.8): they have no free text to protect, so their flags may come before, between or after the words (`md-memo scrap search deploy --limit 5`); `--` still ends the flags, and an undefined `-h` after the words prints that command's usage with exit 0.
...
- Windows PowerShell 5.1 pipes to native programs in the console/ANSI encoding by default (general PowerShell behaviour, unverified in this repo), and `md-memo buffer get | Set-Content` or `> file` re-encode the output the same way; to READ a note with non-ASCII (Japanese) text into a file use `md-memo buffer get --out <file>` (section 1.2), which writes UTF-8 itself. To WRITE such text, non-ASCII content is safer through the JSON-RPC route (section 2) or PowerShell 7 with UTF-8.
...
| `buffer get --out <file>` (with `--selection`, `--tab`, `--bom`, `--json`, `--text`) | `--out` (path), `--bom` (bool) | The CLI process writes the text to `<file>` and prints only what it wrote. JSON: `{path, bytes, hash, generation?}`; text: one line `Wrote N bytes to <path> (hash H)`. `path` is absolute; `bytes` is the size of the file (a BOM counts); `hash` is the note's hash (the value `buffer set --expected-hash` takes; with `--selection` the same function over the selected text) and `generation` is present only for the whole note. | `--out` empty, a directory, or a folder that does not exist: `Error: ...`, exit 1 BEFORE the app is asked. `--bom` without `--out`: error. No selection: `Error: no active selection` and no file is written. |
...
`buffer get --out` details: no new RPC (it is `buffer.get` / `buffer.get_selection` plus a local write). The file is UTF-8 without a byte order mark unless `--bom`; it is written to a temporary file in the same folder and renamed over the target (a reader never sees half a file; an existing file is replaced and keeps its permissions); the path is resolved against the working directory of the `md-memo` process, not the app's; no parent folder is ever created. The text is exactly what `buffer get` would print, so the note's own line endings stay as the app reports them. Reason it exists: text piped through a shell is decoded and re-encoded with the console code page (Windows PowerShell 5.1), which garbles Japanese; a file written by the CLI does not pass through the shell. `(unverified on a real PowerShell 5.1 against a running app)`.
...
Not implemented (design documents only, verified absent from the code): `md-memo share`, `md-memo agent init-skill` (a different idea from `agent install-skill`, section 1.9, which exists), any `md-memo config` action other than `get` (there is no `config set` or `config edit`; `config get` exists, section 1.8), `filters.json`, `prompts.json`, `hooks/`, and reading of `jev.json` / `.jev.json` (the rules engine `jev.NewRules` exists but no production code loads any file; every caller passes `nil`). See `docs/design/agent-malleable-architecture.md`.

### 1.5 `ocr` (image to text, appended to today's scrap)

`md-memo ocr [--json] <imagePath>` (`pkg/cli/ocr.go`; also `md-memo --headless ocr ...`). Windows and macOS. Local like `jev`: no running instance and no GUI are needed, which is what the Windows Explorer Send To entry relies on (section 4.10). Only `--json` is defined (no `--text`, no `--quiet`); flags go before the path; piped or redirected stdout yields JSON by default (section 1.1).

- It reads two things straight from `<cfg>/config.json` and nothing else: the scrap folder (default `~/Documents/md-memo/scraps`; the top-level `scrap_dir` is read first and `scraps.scrapDir` overrides it, exactly as the app does, `setup-guide.md` (b)) and the `vision` object (`baseUrl`, `model`, `apiKey`, `prompt`, `ocrMode`). A missing or unparsable file means the defaults.
- Engines (`ocr.Recognize`): the cloud vision model of `vision.*` first, the on-device engine (Windows only) as the fallback; `vision.ocrMode` `"on-device"` (Windows only) reads with the on-device engine alone and never sends the image out. Empty `ocrMode` = cloud first. On macOS only the cloud model exists. The whole call is limited to 60 s.
- Success: the text is appended to `<scrap dir>/YYYY-MM-DD.md` as one entry (`scrap.AppendRaw`, section 3.7). Text mode prints `OCR text appended to <note path>`; JSON mode prints `{text, appended: true, path}`. If nothing was recognized nothing is written: `(no text recognized)` (JSON `{text: "", appended: false}`), exit 0.
- Errors (exit 1, stderr `Error: ...`): no path (`usage: md-memo ocr <imagePath>`), `cannot read <path>: ...`, `OCR failed: <reason>` (no key, model or network, engine not available), `failed to append to scrap: <reason>`.
- Side effects: the image goes to the configured cloud vision model unless `ocrMode` is `on-device`, and today's scrap file changes (an open note tab for that file is not guaranteed to refresh; `(unverified)`). The Windows GUI-subsystem caveat of `troubleshooting.md` section 1 (output capture in PowerShell) applies.

### 1.6 `info` (where things are, without the app)

`md-memo info [--json|--text]` (`pkg/cli/info.go`; also `md-memo --headless info`). Standalone and read-only: it reads `<cfg>/config.json` through the shared reader (`pkg/cli/config.go`: a missing or unparsable file means the defaults, exactly as `ocr` treats it) and creates NOTHING (not the settings folder, not the scrap folder, not today's file). It takes no argument. Text at a terminal (`key: value` lines), JSON (one object, snake_case keys) when piped or with `--json`.

| Key | Meaning |
|---|---|
| `version` | the app version (`AppVersion`, passed in by `main`) |
| `config_dir`, `config_file` | `<cfg>` and its `config.json` (the file may not exist: the defaults apply) |
| `scrap_dir` | the scraps folder, `~` expanded (top-level `scrap_dir`, overridden by `scraps.scrapDir`; default `~/Documents/md-memo/scraps`) |
| `today_scrap_path`, `today_scrap_exists` | `YYYY-MM-DD.md` of today's local date inside `scrap_dir`, and whether that file exists |
| `inbox_dir`, `inbox_enabled` | the hot folder (`inbox.dir`, default `~/Documents/md-memo/inbox`) and `inbox.enabled`; the folder is reported also when the inbox is off |
| `autosave` | `general.autoSave` (true unless the file says false) |
| `gui_running` | true when `<cfg>/ipc-session.json` names a port that accepts a connection within 200 ms. The file is only READ: unlike `ipc.LoadSession` (which the app-talking commands use) a stale file is NOT deleted, and the pid check is skipped, so a different program that took over a dead app's port would read as running |

It never prints an API key, a token, the session token or a remote URL. Errors (exit 1): an argument, or an unknown flag.

### 1.7 `scrap` (find the user's scraps without the app)

`md-memo scrap path|list|search` (`pkg/cli/scrapcmd.go`). Standalone and read-only; nothing is created. The folder is `scrap_dir` of section 1.6. Dates are `YYYY-MM-DD` (two-digit month and day, a real calendar day), otherwise `Error: invalid date "<value>" for --<flag> (use YYYY-MM-DD)`, exit 1. Flags may come before or after the words (section 1.1); `--` ends them. An empty result is not an error (exit 0).

| Command | Behaviour | Output |
|---|---|---|
| `scrap path [--date D] [--json]` | the path of that day's scrap file (default today), whether or not it exists | The BARE path on stdout, also when piped (so `$(md-memo scrap path)` works; this is the one command that does not switch to JSON when piped). `--json`: `{date, path, exists}` |
| `scrap list [--from D] [--to D] [--lines] [--json\|--text]` | the files named `YYYY-MM-DD.md` directly inside the folder (not sub-folders; other names and folders are skipped), newest first, `--from`/`--to` inclusive. A missing folder is an empty list. | JSON: array of `{date, path, size, modified (RFC 3339), lines?}` (`lines` only with `--lines`, which reads every file: line feeds plus one for a last line without one). Text: `date  N bytes  [N lines]  path` per line, or `No scrap files in <dir>` |
| `scrap search <text...> [--from D] [--to D] [--limit N] [--json\|--text]` | case-insensitive substring search over every `.md` under the folder (sub-folders included, folders starting with `.` skipped; `pkg/search.SearchScrapsOrdered`). Several words are one query joined by spaces; put `--` first for a query that starts with `-`. Files are searched in this order: the daily files newest day first, then any other `.md` by path. It stops after `--limit` matches (default 100, at least 1). With `--from` or `--to` only files named `YYYY-MM-DD.md` inside the range are searched (other `.md` files are excluded). | JSON: `{query, count, truncated, matches: [{file, date?, line, text, heading?, heading_line?}]}`. `file` is a full path, `date` is set when the file name is a date, `line` is 1-based, `text` is the line without its line ending (CRLF files are fine), `truncated` is true when more matches existed than `--limit`. `heading` is the nearest ATX heading (`#` to `######`, at most three spaces of indent, a space or end of line after the hashes) at or ABOVE the line, as the trimmed line including its `#`s, and `heading_line` its line number; a match on a heading line reports that heading itself; no heading above means both are omitted. `#` lines inside fenced code (``` or ~~~) are not headings. Setext headings (underlined) are not recognised. Text: `path:line: text`, then `    under: <heading> (line N)`. |

Text piped in with `cmd | md-memo` is filed under `## [HH:MM:SS] title` (section 3.7; entries appended raw by `ocr`, the hot folder, Discord or Mobile Drop have their own shapes and no such heading), so a hit inside a piped log names the entry it belongs to; read around it with the file path and the line numbers (this is the intended use: `scrap search`, then read the lines). Order differs from the GUI search (Ctrl+Shift+F), which scans in parallel and caps approximately; the CLI's order and cap are exact. The GUI search results are unchanged (the extra fields `heading` and `headingLine` exist on the shared Go type but are only filled for the CLI).

### 1.8 `config get` (the settings, with every secret hidden)

`md-memo config get [<key.path>] [--json|--text]` (`pkg/cli/configcmd.go`). Standalone and read-only. THIS is how an agent may look at the user's settings: `config.json` itself must not be read (`SKILL.md` safety rule 2), because it holds API keys and the Discord bot token in plain text.

- Output: the parsed `config.json` as pretty JSON (keys sorted; numbers exactly as written). No file: `{}`. A file that is not valid JSON (or not an object): `Error: cannot use <path>: ...`, exit 1 (never `{}`: that would read as "nothing is configured"); the error does not echo the file.
- Redaction (`RedactConfig`): every STRING at, or anywhere below, a key whose name contains `apikey`, `api_key`, `api-key`, `token`, `secret`, `password` or `passwd` (case-insensitive; the rule of `pkg/configpack.IsSecretKey`, the same one the settings-package export uses) becomes `"<set>"` when it has a value and `"<unset>"` when it is empty; no part of the value is kept (no last four characters). That covers `text|autocomplete|vision|voice|cli|action|image.apiKey`, `discordBridge.botToken`, and an `env` map of an agent (`OPENAI_API_KEY`). Numbers, booleans and null are kept (`autocomplete.maxTokens` stays a number). Credentials inside a URL are removed: `user:password@` (or a bare `token@`) of an http(s) URL (`configpack.StripUserinfo`), the same for other schemes (`postgres://user:pw@host`; `ssh://git@host` is an identity and stays), and the value of a secret-looking query parameter (`?key=`, `token=`, `api_key=`, `sig=`, `auth=`, ...) becomes `<set>`. The whole document is redacted first and the key path only picks from the redacted copy, so no path reaches a secret.
- Key path: dotted names walk the objects (`scraps.scrapDir`, `vision`, `general.autoSave`); a number picks an array item (`agents.0.name`); a name that itself contains a dot works (the shortest match that resolves is used). An unknown path: `Error: no such key: <path>`, exit 1.
- Format: JSON when piped or with `--json` (a single value is its JSON: `"<set>"`, `true`, `256`); `--text` prints a single value bare (`<set>`, `true`), which is what a script wants; an object or array is JSON in both modes.
- The frontend's own copy of the settings in the WebView's `localStorage` (`md_notepad_config_v3`) no longer holds any key or token either (blanked by `frontend/js/secret_strip.js` on every save; an older build's copy, and the even older `md_memo_config_v1`, are replaced or removed at the first save). `config.json` is still where the keys live; keep treating it as secret.

### 1.9 `agent install-skill` (put this skill where an agent reads it)

`md-memo agent install-skill [--claude | --codex | --dir <path>] [--force] [--link] [--json|--text]` (`pkg/cli/skillcmd.go`, the copying in `pkg/skillinstall`, the embedded files in `skills/embed.go`). Standalone: no running MD-Memo, no network, no agent process, and it never reads `config.json`. Only in builds newer than 1.8.0 (check `md-memo --help`). It writes into the user's agent folder under their home, so run it only when the user asked.

- What it installs: the skill folder that is `go:embed`ded in the binary (`SKILL.md` and `references/`, about 320 KB; a test compares the embedded copy with `skills/md-memo` on disk, so it is what the repository and the release zip carry). The Homebrew cask installs only the `.app`, so this command is how a Homebrew user gets the skill; the zips also carry the folder.
- Target: `--claude` (the default) is `~/.claude/skills/md-memo`, or `$CLAUDE_CONFIG_DIR/skills/md-memo` when that variable is set. `--codex` is `$CODEX_HOME/skills/md-memo`, else `~/.codex/skills/md-memo`: that Codex reads skills from there is `(unverified)`, and the command says so. `--dir <path>` is `<path>/md-memo` (a leading `~` is the home folder; a relative path is taken from the current folder). The three are mutually exclusive.
- How: the files are written into a hidden temporary sibling folder of the target and renamed into place (an agent reading the skills folder never sees half a skill; an existing copy is renamed aside first and removed after the new one is in place, put back if that fails; on Windows a copy that another program holds open cannot be moved, the error says so). Every installed folder gets `.md-memo-skill-version`: JSON with `version` (the md-memo version), `hash` (a SHA-256 over the files, with CRLF read as LF so the same skill has the same hash on every build) and the hash of each file.
- Running it again: identical content is `already up to date` (exit 0; a hand-made identical copy gets a marker); an older copy whose files still match its marker is replaced (`Updated the md-memo skill (it was installed by md-memo X)`); a folder with edited, missing or extra files (relative to its marker), or with no marker and different content, or a file or link where the folder belongs, is left alone: exit 1, the differing files listed as `modified: x`, `missing: x`, `extra: x`, and `--force` to replace it (the lost changes are listed again). A marker from a NEWER md-memo is not downgraded without `--force`. `--force` replaces the whole folder, so files the user added there go too.
- `--link` makes `md-memo` a symbolic link to a real `skills/md-memo` folder found beside the program (up to six folders above it, which covers an unpacked zip and `MD-Memo.app/Contents/MacOS`) or around the current folder (a checkout), so the agent always reads the checkout's text; no marker is written. Not on Windows: a symbolic link needs administrator rights or Developer Mode and a junction cannot be made with the standard library, so `--link` there prints why and exits 1 (install a copy instead).
- Output: text at a terminal (what was installed, where, the file names, the marker), JSON when piped (`{action, path, version, hash, files, bytes, names, target, base}`; `action` is `installed`, `updated`, `replaced`, `up-to-date` or `linked`). Exit 0 ok, 1 error.
...
Source: `pkg/slotagent/parser.go`, `config.go`, `mention.go`, `enabled.go`, `loader.go`, `runner.go`, `pipeline.go`, `skill.go`, `app_slot.go`, `frontend/js/slot_agent.js`, `auto_selector.js`, `slot_snippets.js`.
...
- Ranges inside fenced code blocks (backtick or tilde fences), inline code, Markdown links `[t](u)`, bare `http(s)://` URLs and HTML comments (3.1.4) are never slots. A slot that starts or ends inside one of those ranges is skipped (a slot that holds a whole comment is not: the comment stays part of its text).
- Content starting `⟳`, `実行中...` or `(実行中...)` is a running placeholder and is skipped.
- `@name` (`@name: instruction` or `@name instruction`) is resolved AGENT FIRST (`slotagent.ResolveAgentName`): an exact `agents` key, else an `agents` key compared case-insensitively, else an alias from `aliases` (case-insensitive; a tie is broken by the alphabetically smallest key). On a hit `SlotMatch.AgentName` holds the `agents` key, `SkillName` stays empty and `OutputMode` is `below` (the result goes under the line, see 3.1.1); the profile's system instruction is not applied. Any other `@name` names a skill (`SkillName`, `OutputMode` `replace`). This holds for every non-recipe delimiter pair, not only `{{ }}`; a recipe never takes the mention form. The key of an agent that `enabled: false` / `disabled_agents` switched off (3.1.5) is an agent mention too, checked BEFORE aliases: `SlotMatch.DisabledAgent` holds it, `SkillName` and `AgentName` stay empty, `OutputMode` is `below`, and the run reports the agent as disabled instead of looking for a skill or picking another agent. The aliases of a disabled agent are free again, so `@gemini` with `agy` off is an ordinary name (a skill, or another agent's alias).
...
- Which slot the Go side picks (`ParseSlotsRPC` / `RunSlotAgentAsync`): the slot containing the caret, else the NEAREST slot AFTER the caret, else the FIRST slot in the note; "no slot found" only when the note has no slot at all. Exception: with the caret inside an HTML comment (3.1.4) nothing is picked, not even a slot that holds the comment, and no gate is resumed (`ParseSlotsRPC` answers `caretInComment: true`, no target, `hasWaitingApproval: false`; `RunSlotAgentAsync` sends the empty "completed" result). Ctrl+Enter reaches this rule only when the Auto selector hands over to it: the setting is off, the line is blank, the caret is in a code fence, the caret is inside (or touching) a classic slot, or the line already holds a classic slot notation (section 4.1).
...
- `SlotParseMatch` (JSON): `type`, `openDelimiter`, `closeDelim`, `startOffset`, `endOffset`, `rawContent`, `role`, `skillName?`, `agentName?`, `outputMode` (`replace` or `below`), `instruction`, `isInline`, `isTarget`. The parse answer (`SlotParseResponse`) holds `targetSlot?`, `allSlots`, `hasWaitingApproval`, and `runAgentKey?` / `runAgent?`: the agents key and definition a run at this caret would start (3.1.3; absent when none would), and `runProblem?`: why that run cannot start (agent disabled, none enabled, program not on PATH; 3.1.5), in which case `runAgentKey` and `runAgent` are absent. `SlotExecutionResult` also carries `output` (the agent's stdout, untrimmed), `outputMode` and, for a run that could not start, `problem` (3.1.5); in `below` mode Go replaces nothing (`newContent` equals `oldContent`) and the page writes `output` under the task line.
- Full-width `｛｛`, `［？`, `【？` typed by an IME are converted to `{{`, `[?`, `【?`. Typing an open delimiter (`{{`, `[?`, `【?`, `[!`, `[>>`) opens the quick selector (Up/Down, Tab/Enter, 1-9, Esc): the slot profiles and recipes first, then the snippets of 3.1.2 (kind tags LLM / AGENT / CMD / TEXT). The number keys 1-9 address the first nine rows, so the profiles keep the keys they always had. A profile whose agent is disabled (3.1.5) has no row and takes no number, and a row whose agent's program is not found on PATH carries a short tag "not found" (ja 「未検出」), added after the list is shown (one lookup per agent; 3.1.5). Confirming a profile inserts `{{ code: ` ... ` }}`. The popup's header and key hint follow the UI language (EN "Hand over to an agent"). Typing `[[` opens nothing (wiki links are not disturbed).
...
Before any run, whatever started it, a run that cannot start at all is refused without touching the note: its agent is disabled, every agent is disabled, or its program is not on PATH (3.1.5). Then an agent that acts without asking for permission is confirmed first and nothing in the note changes until the user says Run (3.1.3).
...
- One task per line: the notation must close on the same line. The line may start with an indent, a `>` quote prefix and/or a list marker (`- `, `* `, `+ `, `1. `, `1) `, a checkbox `- [ ] `, `・`); a rewrite keeps that prefix. Not recognised inside code fences or inline code, nor inside an HTML comment or cut by one (it starts or ends inside a comment; 3.1.3).
- The caret is "in" a task from its first bracket to just after its last one. When the task is all that is on its line (after the prefix; HTML comments on the line do not count) the caret may be anywhere on that line. Ctrl+Enter runs exactly ONE task: the one under the caret; with a selection the one under its start, else the first one inside it.
...
- The markers are plain HTML comment text, but they are structure, not hidden text: the comment rules of 3.1.4 do not treat them as comments (a result block is still found and skipped as one), and Ctrl+/ never wraps or unwraps a marker line. The editor shows them; the preview hides them (`AutoSelector.stripMarkers` removes the `md-memo:run`, `md-memo:res` and `/md-memo:res` lines before markdown-it renders, after fenced code has been set aside; markdown-it itself runs with `html: false`, which would otherwise print them as literal text).
...
- A failure is one line inside the block: `[LLM error: <message>]` (ja UI `[LLMエラー: <message>]`, message cut to 300 characters) or `[<agents key> error: <message>]` (ja `[<key> エラー: <message>]`). The agent message is the runner's own (3.1 step 4: `エージェント起動失敗: ...`, stderr, `Exit Code N`, timeout). The runner starts its failure text with the warning sign and `エラー:` (a classic slot shows it as it is); the block already says `error:` / `エラー:`, so it drops that lead-in (`[U+26A0] エラー:`, spaces and a full-width colon allowed; only at the start, once) and shows `[claude-code エラー: rate limit exceeded]`, not `[claude-code エラー: [U+26A0] エラー: rate limit exceeded]`. A message that is only the lead-in, or none, becomes `Exit Code N`.
...
- Finding result blocks (`frontend/js/result_blocks.js`, `window.ResultBlocks`: pure functions over the note text, called by the page only). `findResultBlocks(text)` lists them as `{ id, closed, openLine, closeLine, openStart, openEnd, bodyStart, bodyEnd, closeStart, closeEnd }`: a whole opener line `<!-- md-memo:res <id> [attrs] -->` (indent and trailing blanks allowed, LF or CRLF) and the FIRST closer line `<!-- /md-memo:res -->` after it. A marker inside a fenced code block (```` ``` ```` or `~~~`) is only text. An opener with no closer before the next opener or `md-memo:run` marker, or before the end of the note, is an unclosed block (a run that never finished): only its opener line is known. What lies between two markers is not searched for fences, and a fence that starts there does not hide the closer.
- The gutter shows them: a 3px bar at the left edge of the line-number gutter (both editors; it scrolls with the numbers and never touches a digit): bright green on the opener line, light green on the lines of the result, dark green on the closer line (an unclosed block: only the bright bar). Colors are `--result-open`, `--result-body` and `--result-close` in `style.css`, one set per theme. Only the gutter is drawn: nothing is painted behind or over the text. A note with no block has no overlay element at all; the bars follow zoom and wrapped lines like the numbers (3.8).
- Five commands work on them: palette entries "Result: Go to Next Result Block", "Result: Go to Previous Result Block", "Result: Copy This Result", "Result: Delete This Result", "Result: Confirm This Result" (ja 「結果: 次の結果ブロックへ移動」 / 「前の結果ブロックへ移動」 / 「この結果をコピー」 / 「この結果を削除」 / 「この結果を確定」), and rebindable shortcuts `resultNext`, `resultPrev`, `resultCopy`, `resultDelete`, `resultConfirm` (4.1; none has a default key). Next / previous move the caret to the start of the next / previous opener line by opener position and wrap around the ends of the note (status "Result block N of M"; "This note has no result block" when there is none). Copy, delete and confirm act on the block that holds the caret (its marker lines count); with the caret outside every block the status is "The caret is not inside a result block", and an unclosed block is refused ("This result block was never closed ..."). Copy puts the body without the two marker lines on the clipboard. Delete removes the whole block, closer line and its line break included (for a block on the last line of the note, the line break before it instead), in ONE undo step. Confirm replaces the block by its body lines in ONE undo step: the marker lines go, the text stays, and a task notation in it is live again (the palette description says so). While the preview covers the editor the commands only show "The preview is showing: switch to the editor first".
...
- `agents.<name>.aliases`: a list of extra names accepted after `@`, compared case-insensitively; an `agents` key beats an alias (rule in 3.1). When omitted, the built-in agents keep their default aliases: `claude-code` -> `claude`, `cc`; `agy` -> `antigravity`, `gemini`; `hermes` and `codex` have none. A default alias that is already another agent's key or explicit alias is skipped, and an explicit list (even `aliases: []`) replaces the defaults. `CheckAgentAvailability` (the "agent not found" warning, the Settings availability badge) resolves aliases too. A disabled agent takes its aliases with it (3.1.5).
...
#### 3.1.3 Agent safety: the confirmation, `append_instruction`, agent definitions to review

Source: `frontend/js/agent_risk.js`, `pkg/slotagent/safety.go`, `app_agent_safety.go`, `slot_agent.js` (`allowAgentRun`), `app.js` (`confirmAgentRun`, `customConfirm`).

Confirmation before a risky agent runs. Every run goes through it: a classic slot, a `{{ @agent }}` task, a slot bound to a profile, a recipe or a resumed approval gate, the Auto selector's agent tasks, and a slot a Quick Actions card inserts. `ParseSlotsRPC` names the agent the run would start (`runAgentKey`, `runAgent`; `slotagent.RunAgentFor`: the `@agent`; else the notation's profile agent; else `default_agent`; an unknown agent or one without a command falls back to `default_agent`, then to the built-in `claude-code` (never a disabled agent: 3.1.5); a recipe and a resumed gate use `default_agent`; an `@agent` is never swapped). A definition is risky when:
- an argument is one of `--dangerously-skip-permissions`, `--allow-dangerously-skip-permissions`, `--dangerously-bypass-approvals-and-sandbox`, `--yolo`, `--full-auto`, `--auto-approve` (the whole argument, or `<flag>=<value>`; any case), or
- the instruction is appended (no argument holds `{instruction}` and `append_instruction` is not `false`) and the command is a shell: its program name (folder and `.exe` dropped, any case) is `powershell`, `pwsh`, `cmd`, `sh`, `bash` or `zsh`, or an argument is `-Command`, `/c` or `-c` (any case). Such a program joins the arguments after its command switch into one command line, so the appended note text would run as code.

Then, before the note is touched, a dialog names the agent, each flag found ("runs without asking for permission to use tools or change files") or the shell, and the command line. Buttons Run / Cancel (ja 「実行」/「キャンセル」); Cancel has the focus, Enter presses the focused button, Ctrl/Cmd+Enter is ignored while it is open, Esc cancels. Cancel runs nothing and leaves the note as it was: no placeholder, no run marker, and the Auto selector's rewrite is not made (with `autoSelector.agentConfirm: false` it asks before rewriting; if the backend then resolves another, risky definition and the user declines, the rewritten line is put back). Run is remembered per agents key and exact command line in `config.json` `agentAck` (`{"<key>": "v1:<53-bit hash of command, args and append_instruction>"}`): the same definition is not asked about again, and any change to the command, an argument or `append_instruction` asks again. It is a disclosure, not a sandbox: a flag not on the list is not caught, and whoever can edit the agents file can change what runs. The Settings warning under the Agent select ("This agent is configured to modify files and run commands without asking") uses the same flag list.

`agents.<name>.append_instruction` (bool, default: absent = `true`): `false` means the instruction is never appended when no argument holds `{instruction}` (for an agent that works from `{file}` alone, and for any shell command line). `true` or absent keeps the rule of 3.1 step 3.

Agent definitions to review (`agent_issues` in `GetActiveSlotConfigJSON`, a list of `{agent, kind, detail, suggested?}`; absent when empty). The Go side checks the resolved agents (the agents file, or the built-in defaults) and any `agents` in `config.json`:
- `kind: "outdated-default"`: the definition equals (command and arguments exactly) one MD-Memo shipped up to v1.8.0 as the Go default, the page default or the agents.yaml template (`legacyAgentDefaults`): `claude` `--file {file} --prompt {instruction}` (Claude Code has no `--prompt`, and its `--file` takes `file_id:path`), `codex` `--execute --file {file}` (Codex CLI has neither option), `agy` `-p "対象ノート: {file}\n指示: {instruction}" --dangerously-skip-permissions` and `agy` `-p {instruction} --dangerously-skip-permissions`. `detail` is the built-in agent, `suggested` today's definition (`DefaultSlotConfig`). Reported under whatever key holds it.
- `kind: "shell-append"`: the shell rule above; `detail` is the shell or the switch found.
- `kind: "default-disabled"` (3.1.5): `default_agent` names an agent that is disabled; `agent` is that key and `detail` the agent that is the default instead (`""` when every agent is disabled). The text in Settings -> Agent: "agy: default_agent names this agent, but it is disabled in agents.yaml (enabled: false or disabled_agents), so claude-code is the default agent."

Where it shows: Settings -> Agent, under the Agent select, a box "Agent definitions to review" (ja 「確認が必要なエージェント定義」) with one line per issue, a "Copy the new definition" button (ja 「新しい定義をコピー」) for an outdated one (it copies a YAML entry under the user's key with today's `command` and `args` and the user's description and aliases), and "Hide until this list changes" (`config.json` `agentNotice.dismissed`); and once per set of issues a status-bar message 4 s after start-up, "N agent definition(s) need a look: see Settings > Agent." (ja 「確認が必要なエージェント定義が N 件あります（設定 → 連携）。」, `agentNotice.shown`). The app never rewrites the agents file: the user edits it (or deletes an entry to get the built-in one back).

Built-in agents since the change after v1.8.0 (the same in `DefaultSlotConfig`, `slot_agent.js` and the template): `claude-code` `claude -p "対象ノート: {file}\n指示: {instruction}"`, `codex` `codex exec {instruction}`, `agy` `agy -p "対象ノート: {file}\n指示: {instruction}"` (no `--dangerously-skip-permissions`, so agy may stop to ask for permissions in print mode; a user who adds the flag back is asked once before it runs), `hermes` unchanged. None of these command lines was run on the maintainer's machine: `codex exec [PROMPT]` and `agy -p` (print mode) are what their `--help` lists there (checked 2026-09-25; `codex exec --help` also lists `--skip-git-repo-check`, "Allow running Codex outside a Git repository"), and `claude` is not installed there, so `claude -p` rests on the maintainer's report of Claude Code's print mode.

Known false positives of the shell rule: some agent CLIs use `-c` for something else (Codex CLI `-c key=value` is `--config`, agy `-c` is `--continue`). Such a definition is flagged only while the instruction is appended; writing `{instruction}` into its `args` (the built-in definitions all do) clears it.

#### 3.1.4 HTML comments: hidden text that never runs

Source: `frontend/js/html_comments.js` (`HtmlComments`), `pkg/slotagent/comments.go` (the same rules on byte offsets; `tests/fixtures/html_comment_vectors.json` is run by both test suites), `frontend/js/comment_toggle.js` (Ctrl+/).

What counts as a comment:
- `<!--` up to the FIRST `-->` after it; comments do not nest. `<!-->` and `<!--->` are complete, empty comments (as in HTML). A `<!--` with no `-->` anywhere after it is not a comment, and nothing after it is one either.
- A `<!--` inside a fenced code block (at most 3 spaces, then 3 or more backticks or tildes; closed by a line of the same character at least as long with nothing else on it; an unclosed fence runs to the end of the note) or inside inline code does not start a comment. Inline code is read on one line: a run of N backticks opens a code span when a later run of exactly N backticks on the same line closes it, otherwise it is plain text. Once a comment has started, fences and backticks inside it mean nothing.
- md-memo's markers (`<!-- md-memo:run ... -->`, `<!-- md-memo:res ... -->`, `<!-- /md-memo:res -->`, i.e. `<!--`, optional whitespace, optional `/`, `md-memo:`) are not comments in this sense (3.1.1).

What that changes:
- Nothing inside a comment runs: slots of every notation (3.1), task notations (3.1.1) and approval gates are skipped by the Go parser (`FindExcludedRanges` step 6, `FindApprovalGates`), by `AutoSelector.findTaskAt`, by the run button and by the Auto selector. A notation that starts or ends inside a comment is skipped too (the rule of `isOffsetExcluded`, applied the same way on both sides so that the page never hands Go a slot that Go would skip). A notation that holds a whole comment is still a notation, and the comment stays part of its instruction.
- The caret strictly inside a comment (between its `<` and its last `>`): the run button is not shown, and Ctrl+Enter and `SlotAgent.triggerSlotExecution` (used by Quick Actions) do nothing but show the toast "Inside a comment (<!-- -->): nothing runs here." (ja 「コメント (<!-- -->) の中なので実行しません」). The same when the target (the selection, else the current line) is only commented text and spaces. They never fall back to another slot.
- The preview does not show comments: before code is set aside, `HtmlComments.removeComments` takes every comment out; a line left with nothing but spaces (or `>` quote marks) goes with its line break, so the paragraph around it stays one paragraph; a comment inside a line is cut out in place. Code, markers and an unclosed `<!--` are shown as before.
- A plain sentence inside a comment is not an instruction: the Auto selector judges the line without its comments, and a line that holds a comment is never rewritten into a task (the ask bar opens instead).

Ctrl+/ (Cmd+/ on macOS; `shortcuts.commentToggle`, palette "Toggle comment"): toggles the lines the selection touches (a selection that ends at the start of a line leaves that line out; no selection = the caret's line) as ONE undo step. `general.commentStyle`:
- `line` (default): each line becomes `<!-- text -->`, its indentation kept in front (`  - item` -> `  <!-- - item -->`). When every line that can be toggled already is a one-line comment, they are all uncommented (one space inside each delimiter is removed with it); otherwise the other lines are commented. Empty lines and marker lines are never touched. A line that contains `-->`, or that is part of a comment spanning several lines, is left as it is; the toast names it: "Not toggled (contains --> or is in a comment): line {lines}".
- `block`: `<!-- ` before the first non-blank character of the lines and ` -->` after the last one; when the lines are exactly one comment it is taken off (a `<!--` or `-->` alone on its line goes with that line). Refused, with a toast, when the lines contain `-->` or a marker, or touch a comment that goes on outside them.
- Both: the result is checked with the comment rules above. Nothing changes, with a toast, when an unclosed `<!--` comes before the lines (a new `-->` would close it and hide everything in between) or when the lines are inside code (there `<!--` is plain text) or the edit would change other comments. LF and CRLF line ends are kept.
...
- `disabled_agents` (top level, a list of agents keys; any case, blank items ignored): the same for several agents at once. Either spelling works and they are added together; `enabled: true` does not undo a listed key.
- Deleting an entry is NOT the same: a built-in agent (`claude-code`, `hermes`, `codex`, `agy`) that the file does not define is added back at every load. Switch it off instead.

What a disabled agent is. One step, `finalizeAgents`, ends `complementSlotConfig`, `MergeSlotConfig` and `App.buildActiveSlotConfig`, so every consumer sees only enabled agents; `frontend/js/slot_agent.js` applies the same rules to the page's own copy after every merge into it.
- It is left out of `agents`, never added back from the built-in defaults, and the definitions the page sends cannot bring it back (the agents file decides). Its aliases go with it: turning `agy` off frees `gemini` and `antigravity`. `GetActiveSlotConfigJSON` carries the sorted keys as `disabled_agents` (left out when none).
- It is never chosen: not by the Auto selector (a line that asks for an agent runs the default agent), not offered by the quick selector (a profile whose agent is off has no row and takes no number key; recipes run the default agent), not picked for a snippet (`pickAgent`: the snippet's `agent`, else `default_agent`, else the first agent, among the enabled ones), not a profile's or a recipe's agent.
- `default_agent` that names a disabled agent (or, with `claude-code` disabled, names nothing usable, which would fall back to the built-in `claude-code`) becomes the first enabled agent that has a command in the order `claude-code`, `hermes`, `codex`, `agy`, then the other keys alphabetically; `""` when there is none. The file is not rewritten. `agent_issues` (3.1.3) gets `kind: "default-disabled"`.
- Named anyway, `{{ @agy ... }}` (the key, any case) or a profile whose `agent` is off is a run that cannot start (below): never "skill not found", never another agent's run, and the note is not rewritten. A recipe and a resumed gate use the default agent. With every agent disabled every run says `none-enabled`.
- Settings -> Agent lists the enabled agents as `key - description` (just the key when there is no description) and, when any is off, a line under the list "Disabled in agents.yaml: agy, hermes" (ja 「agents.yaml で無効: agy, hermes」); with every agent off the select is disabled and reads "No agent is enabled". The agents import (`ImportAgentsConfigFile`) keeps a disabled definition in the copy it saves (marked `enabled: false`).

A run that cannot start. Before any process is started and before the note is touched, `slotRunProblem` checks the agent the run would use (the same choice as `RunAgentFor`, 3.1.3). The message is worded in English and in Japanese by the page; `message` holds the English sentence, then ` / `, then the Japanese one:

| `kind` | When | English sentence |
|---|---|---|
| `disabled` | `@agy`, or a profile, names a disabled agent | `Agent "agy" is disabled in agents.yaml (enabled: false)` |
| `none-enabled` | every agent is disabled | `No agent is enabled: every agent is disabled in agents.yaml (enabled: false)` |
| `missing` | the agent's program is not found on PATH (`exec.LookPath` through the 30 s cache, and once more without it when the cache says no, so a program installed a moment ago is not refused). Only a bare program name or an absolute path is checked; a relative path with a folder in it (`tools/agent`) is resolved from the project folder by the run and is left to it | `Agent "hermes" needs "ollama", which was not found in PATH. Install it or choose another agent in agents.yaml.` |

Where it shows: `ParseSlotsRPC` answers `runProblem: { kind, agent?, command?, message }` and the page shows the sentence as a status-bar message and stops: no placeholder, no run marker, no rewrite, no confirmation dialog. `RunSlotAgentAsync` makes the same check before it writes anything or starts anything and, for any caller that skipped the page's check, answers a `failed` result with `problem` and `newContent` equal to `oldContent`; the page then takes away what it had put in for the run (the running placeholder, or the marker under a task), as for a cancel, marks the task failed and tells the user. A plain line `@agy ...` typed for the Auto selector says the same (`classify` reason `disabled-agent`). Only the PROGRAM is looked up: nothing is claimed about what it needs to work (an Ollama model that is not pulled yet, a login), and `ollama run` may start downloading a model by itself. An agent with no `command` at all is still the run's own error.
...
`[label](target)` and `![label](target)` on one line. `target` may be a `file://` URL, an absolute path, or a path relative to the note's folder (or the open workspace folder); it may be wrapped in `<...>`. An `http(s):` target (in a Markdown link or image, or a bare `http://` / `https://` address in running text; the bare form is ASCII only, so Japanese text right after it is not swallowed) is a web address: Ctrl/Cmd+Click opens it in the browser and Alt/Option+Click does nothing for it. `mailto:` and any other scheme is neither underlined nor opened.
- Ctrl+Click (Cmd+Click on macOS) with the caret on the link: opens with the OS default app (`rundll32 url.dll,FileProtocolHandler` / `open` / `xdg-open`); a web address opens in the browser. Alt+Click (Option+Click) without Ctrl/Cmd: reveals in Explorer (`explorer.exe /select,`) / Finder (`open -R`). A file target must exist. Only plain paths, `file:` URLs and http(s) addresses are accepted; other schemes are refused.
- Underline marks (`file_anchor.js`): every link that Ctrl/Cmd+Click can open is underlined in the editor (a thin line, dotted for a local image, which previews on hover), drawn by a transparent copy of the note behind the textarea; the rendered preview is unchanged. The layer is built only for notes of up to 100,000 characters (`MARK_MAX_CHARS`) that contain a link, with at most 3,000 marks; a bigger note stays unmarked but Ctrl+Click still works. Marks are refreshed when typing pauses (100 ms; at most 400 ms of continuous typing). While Ctrl/Cmd/Alt is held the link under the pointer gets thicker and the pointer becomes a hand; resting on a link for about 300 ms shows a hint ("Ctrl+click to open / Alt+click to show its location", or "Ctrl+click to open in the browser" for a web address). Verified in Chromium only; not yet on the macOS WebView (`(unverified)`).
...
### 3.7 Entries written by Quick Capture, Screen Capture, the hot folder and `md-memo ocr`

Sources: `quickcapture.go`, `quickcapture_windows.go`, `imgcap.go`, `imgcap_windows.go`, `app_inbox.go`, `pkg/ocr/ocr.go`, `pkg/cli/ocr.go`. All of them append to the END of `<scrapDir>/YYYY-MM-DD.md` (local date) as separate entries divided by a blank line; an open note view of that file updates at once (the `ocr` CLI runs in its own process, section 1.5). The fixed header and failure texts below are Japanese in every UI language.

- Quick Capture, Send: the typed text exactly as typed, or the clipboard text when the field is empty; no context line. AI Send (and Screen Capture, below) start the entry with the line `> [context: <title of the window you were in>]` when the popup was opened by its global hotkey; from the toolbar, palette or tray there is none, because MD-Memo itself is in front. AI Send first runs the text through the Alt+C correction prompt on the `text.*` model, only while `general.aiCorrection` is not false and `text.baseUrl` is set; any error, an empty answer or 30 s without an answer saves the text as typed.
- Screen Capture (one entry per capture session, the PNGs saved BEFORE OCR starts):

```
**[18:12:15] 画面キャプチャ**
<caption typed in the popup, if any>
> [context: <window you were in>]                (only when the popup was opened by the hotkey)

**1. <title of the window>**                     (a free rectangle: "1. 選択範囲 (200×100)", width x height)
![capture 1](./assets/2026-09-24-181328-cap1.png)
> recognized text, one blockquote line per line
```

Screen Capture details: window titles are kept up to 400 characters (an Explorer window's title is its full folder path). If OCR fails the text lines become `> (OCRでテキストを抽出できませんでした: <reason>)`. The PNG is `<scrapDir>/assets/YYYY-MM-DD-HHMMSS-cap<N>.png`.

- Hot folder, image: the OCR text as a blockquote (`> ` before every line), then `![name](./assets/<YYYY-MM-DD-HHMMSS>-<name>)`; on failure the line `> (OCRでテキストを抽出できませんでした: <reason>)` instead of the text. Audio: `**[HH:MM:SS]**`, the transcript as a blockquote and a link to the audio file; on failure `> (文字起こしに失敗しました: <reason>)`. The processed file is MOVED, never deleted (also when OCR or transcription fails), to `<scrapDir>/assets/<YYYY-MM-DD-HHMMSS>-<original name>`.
- `md-memo ocr` and the Send To entry: only the OCR text as a blockquote (`ocr.FormatEntry`: every line prefixed `> `); no header, no image link, and nothing is written when no text was recognized.

### 3.8 Wrapped-line numbers

The line-number gutter puts each number on the FIRST screen row of its line; the extra rows of a wrapped line stay blank, so the numbers match the real lines (before, every wrapped row took a number). It follows zoom, window or pane width and the split view's second editor. Notes over 120,000 characters keep the plain `1..N` numbering, so the numbers can drift there once lines wrap. Verified in Chromium only (`(unverified)` on the macOS WebView). The same gutter carries the result-block bars (3.1.1): they take their rows from the same line height and wrapped-row counts.
...
| quickCapture (OS-wide; opens the Quick Capture popup, section 4.10; label "Quick Capture (global, opens the note popup)"; Windows only) | Ctrl+Shift+Q (empty string = no hotkey) | empty: not offered, the Settings row and the toolbar button do not appear |
...
| resultNext / resultPrev / resultCopy / resultDelete / resultConfirm (group "Result Blocks", 3.1.1; work only while an editor has focus; Alt+Shift+Up/Down were not used as defaults because duplicateLineUp/Down own them) | unassigned | unassigned |
| commentToggle (label "Toggle Comment"; group Line Operations; 3.1.4; ignored while an IME composes) | Ctrl+/ | Cmd+/ |
...
2. The caret is inside a result block (3.1.1): the toast "This is a result block. Write your instruction outside of it." and nothing else. Likewise the caret inside an HTML comment, or a target that is only commented text (3.1.4): the toast "Inside a comment (<!-- -->): nothing runs here." and nothing else (never the classic rule of steps 4 and 5).
...
- `autoSelector.agentConfirm`, label "Confirm before an auto-detected agent or command runs" (hint "The line is rewritten first; press Ctrl+Enter again to run it (Ctrl+Z undoes the rewrite)."): on, the agent and command branches stop after the rewrite; off, they run at once after it (a command still passes the guard first; an agent that acts without asking is confirmed before the line is rewritten, 3.1.3).
...
| Inside a comment (<!-- -->): nothing runs here. | コメント (<!-- -->) の中なので実行しません |

### 4.2 Command palette (Ctrl+Shift+P)

Substring filter (case-insensitive) over each entry's title and description; Up/Down/Enter/Esc. Entries: New Tab, Open File, Open Folder, "Ask AI", "Command Bar" (opens in the mode used last), "Command Bar: Run a Command" (manual CLI mode), "Command Bar: AI Writes the Command" (AI mode), "Insert task snippet" (ja 「タスクのひな形を挿入」: the snippet list of 3.1.2 at the caret), Mobile Drop, Voice Input, "Quick Capture" (Windows only: it exists only where the backend offers `openQuickCapture`; ja クイックキャプチャ), "Open inbox folder" (ja 「受信フォルダを開く」; only while `inbox.enabled` is true, both platforms), three prompt presets (Polish, Bullet points, Action items: they open the Ask AI bar with the instruction pre-filled), "Diagram: Convert Selection to Mermaid", "Diagram: Generate Image with Gemini", "Diagram: Generate Image Prompt from Mermaid", AI proofread, "Toggle comment" (ja 「コメントの切り替え」, Ctrl+/, 3.1.4), Export plain text, Zen mode, Full screen, Toggle split (22 commands on macOS with the hot folder off; Windows adds Quick Capture, and the hot folder adds Open inbox folder); plus every note found in the open workspace folder (title, relative path and first line; Enter opens it in a new tab). It has no settings entries.

Five more entries, on every platform and not counted above, work on result blocks (3.1.1): "Result: Go to Next Result Block", "Result: Go to Previous Result Block", "Result: Copy This Result", "Result: Delete This Result" and "Result: Confirm This Result". Each description ends with the action's current key in brackets, and the bracket is dropped while the action has none (the default). The Confirm description warns that a task notation in the kept text becomes live again.
...
1. General: theme (`olive`, `blue`, `forest`, `charcoal`), language (`ja`/`en`), restore session, 2-pane on startup, tray resident, autosave, IME Guardian, AI correction, cursor aura, paste as Markdown, "Toggle comment writes" (`general.commentStyle`: `line` "One comment per line", the default, or `block` "One comment around the lines"; 3.1.4), toolbar and right-click layout editor (show/hide/reorder; the Settings icon cannot be hidden).
2. AI Models: Ollama status/start/stop/"Install Gemma 4" card; Text LLM; Ghost Text; Image OCR/Vision (+ paste-OCR toggle); Voice input (model with suggestions, API style, language codes, mode, custom vocabulary, silence timeout, prompt; there is no voice key/URL field, it falls back to the Vision ones), then the block "Speech engine (hot folder & Discord)" (`voice.engine`: "Gemini (cloud)" or "Whisper (on this PC, offline)", with the Whisper panel of section 4.10; where on-device Whisper is not available it says "On-device Whisper is only available on Windows (x64) for now."); Image generation.
3. Agent (ja 「連携」; four groups, in this order): Commands (Run): CLI model, open result in new tab, Max Pipe Input Size (stored but not enforced); Agents (Delegate): Open agents.yaml button + availability badge, default agent (with the auto-approve warning and the box "Agent definitions to review", section 3.1.3), timeout, Ghost Diff duration, Hover Peek; Auto selector (Ctrl+Enter) (ja 「自動セレクター (Ctrl+Enter)」): two checkboxes, "Let Ctrl+Enter decide: ask the AI, hand over to an agent, or run a command" (`autoSelector.enabled`) and "Confirm before an auto-detected agent or command runs" (`autoSelector.agentConfirm`), both ticked by default and saved with Save like the rest (behaviour in section 4.1); Suggestions (Quick Actions): enabled, manual only, delay, base URL/model/API key; on Windows a fifth group "OS Integration (Send To)" (ja 「OS連携（送る）」): the "Send To menu (image -> OCR)" row with an Installed / Not installed badge and Add / Remove buttons (section 4.10). The group is hidden where the backend has no Send To (macOS).
4. Sync: scraps folder (+ Browse), Git sync toggle/debounce/branch, Git remote URL with Test Connection and Link/Init, git repo status badge, the Discord bridge, and the section "Hot Folder (auto OCR & transcription)" (ja 「ホットフォルダ（自動OCR・文字起こし）」): a checkbox (`inbox.enabled`), the watched folder path with Browse (`inbox.dir`) and, on Windows only, the checkbox "Keep images on this PC: read them with the on-device OCR only (less accurate)" (`vision.ocrMode`), section 4.10.
5. Shortcuts: table of section 4.1 (Windows adds the row "Quick Capture (global, opens the note popup)").
...
### 4.10 Quick Capture popup, Screen Capture, hot folder, on-device Whisper, Send To

Sources: `quickcapture.go`, `quickcapture_windows.go`, `imgcap.go`, `imgcap_windows.go`, `app_inbox.go`, `pkg/inbox`, `pkg/ocr`, `app_speech.go`, `pkg/speech`, `pkg/components`, `sendto_windows.go`, `window_windows.go`, `frontend/js/{app,speech_settings}.js`. None of it is driven by the CLI or RPC; only the user starts the popup and switches the folder on. What the entries look like: section 3.7. Never claim a "no" of this matrix works on macOS.

| Feature | Windows | macOS |
|---|---|---|
| Quick Capture popup, its global hotkey, toolbar button (`btn-quick-capture`), tray item, palette entry | yes | no: the toolbar button, the palette entry and the shortcut row do not appear |
| Screen Capture | yes | no (not implemented) |
| Hot folder (`inbox.*`) | yes | yes, but images are read by the cloud vision model only and audio by Gemini only |
| `vision.ocrMode` `on-device` ("Keep images on this PC") | yes | hidden (no on-device OCR engine); Settings Save never writes the key there |
| On-device Whisper (`voice.engine` `whisper-local`) | yes, x64 only | no |
| Send To entry, tray item "Open inbox folder" | yes | no (no tray, no Send To); the palette entry "Open inbox folder" exists on both |
| `md-memo ocr` | yes | yes, cloud OCR only |
| Link underline and Ctrl/Cmd+click on web addresses (3.3), wrapped-line numbers (3.8) | yes | same web code, not verified on a real Mac |

**Quick Capture popup (Windows).** A small always-on-top native popup for jotting a note without opening the app: one row with a text field and three buttons Send, AI Send and Capture (English labels in every UI language); dark, rounded, about 85% opaque, the Send button in the theme's accent color. Ways in: the global hotkey `shortcuts.quickCapture` (default `Ctrl+Shift+Q`, section 4.1), the toolbar button after the microphone button, the tray menu item "Quick Capture" (English in every UI language, the key shown beside it), the palette entry "Quick Capture". Keys inside it: Enter = Send, Ctrl+Enter = AI Send, Ctrl+Shift+Enter = Capture, Esc = close; while a Japanese IME conversion runs, Enter and Esc go to the IME first. Send appends the text as typed (clipboard text when the field is empty); AI Send corrects it first (3.7; the button reads "Working..." meanwhile, and closing the popup early does not lose the note). The popup closes by itself 1 second after it loses focus (if focus returns in time it stays). Entries go to today's scrap file (`scraps.scrapDir`, 3.7).

**Screen Capture (Windows).** Started by the Capture button or Ctrl+Shift+Enter in the popup; text already typed becomes the caption. Nothing is captured or read from the screen until the user has chosen: the screen dims a little, the pointer becomes a crosshair, a hint bar shows at the top of the main monitor. Hover a window (green frame) and click to capture it, or drag anywhere for a rectangle; holding Ctrl collects several picks (numbered badges; release Ctrl to capture them all; Ctrl+click on a picked window removes it). Esc or a right-click cancels and saves nothing; the picker gives up by itself after 2 minutes. Pickable: visible top-level windows (not minimized, not on another virtual desktop, not the desktop background, not under 24 px, not click-through overlays); the frame is the visible edge without the invisible resize border. A window that is partly covered or partly off screen is captured from the window itself (PrintWindow), so the covering window does not appear; every other window and every rectangle is read from the screen; protected or video content can come out black. Each capture is saved as a PNG under `<scrapDir>/assets/` and then read by OCR one after another in the order picked, with the same engines as the hot folder (there is no progress display, a few seconds). Not available: picking a page element inside a web page.

**Hot folder (`inbox`).** `inbox.enabled` (default false) and `inbox.dir` (default: empty = `~/Documents/md-memo/inbox`; `~` = the user's home), Settings -> Sync. Go reads the keys at start and again whenever a Settings Save changes them (`InitInboxWatcher`), creating the folder if it is missing. Files already waiting are handled once when the watcher starts (at app start, or when a Settings Save switches it on or changes the folder), then new files as they arrive (1.5 s after their last write; the folder is watched non-recursively, subfolders are ignored). Images `.png .jpg .jpeg .bmp .gif .webp` go through OCR, audio `.mp3 .wav .m4a .ogg .flac` through transcription (3.7 shows the entries); the file is moved to `<scrapDir>/assets/` and never deleted, even on failure. The `vision` and `voice` settings are read each time a file is handled, so a changed key or OCR mode applies to the next file. OCR: the cloud vision model of `vision.*` first, the on-device engine (Windows) as the fallback; `vision.ocrMode` `"on-device"` uses the on-device engine only and never sends the image out (the same setting governs Screen Capture, Send To and `md-memo ocr`); empty = cloud first (far more accurate). Audio: `voice.engine` decides (below); the voice settings reuse `vision.baseUrl`/`vision.apiKey` when they have none of their own, so a Gemini key under Image OCR is enough. Opening the folder: the tray item "Open inbox folder" (Windows, only while the hot folder is on) or the palette entry of the same name (both platforms, only while on).

**Speech engine and on-device Whisper (Windows x64).** `voice.engine`: `""` or `"gemini"` (default; everything in `setup-guide.md` (b) Voice input) or `"whisper-local"`. It applies to the hot folder and the Discord bridge only; live voice input (Ctrl+Shift+R) and Mobile Drop keep using Gemini. `voice.whisper` (`llm.WhisperSettings`): `model` (a catalog id or `custom-path` / `custom-url`; blank = the default), `modelPath` (custom-path: a ggml `.bin` on the PC), `customUrl` (custom-url: an https URL) and `customSha256` (optional check of that download), `language` (empty or `auto` = detect; e.g. `ja`, `en`), `threads` (0 = automatic), `prompt` (initial prompt: names and terms to favour), `cloudFallback` (default false: when true and the local engine fails or is not installed, the audio is retried with Gemini, which sends it out). Two parts are downloaded on demand, only from the Settings panel (Download / Remove / Cancel buttons, a progress line, a cancelled or broken download resumes from its `.part` file): the Whisper program (whisper.cpp release b5130 `whisper-bin-x64.zip` from `github.com/ggml-org/whisper.cpp`, about 9 MB, SHA-256 checked, unpacked to `whisper-cli.exe`, `whisper.dll` and `ggml*.dll`) and one model from `huggingface.co`, each checked against a pinned SHA-256. Catalog ids: `kotoba-v2.0-q5_0` (kotoba-whisper v2.0 quantized, Japanese-specialised, about 513 MB, the default), `kotoba-v2.0` (full precision, about 1.4 GB), `large-v3-turbo-q5_0` (multilingual, about 547 MB), `small-q5_1` (about 181 MB), `base` (about 141 MB, lowest accuracy). Stored in `<UserCacheDir>/md-memo/components/` (Windows `%LOCALAPPDATA%\md-memo\components`; the panel shows the exact path). m4a, mp3 and similar are converted on the PC before recognition. Nothing is sent out unless `cloudFallback` is on. On any other platform the panel says "On-device Whisper is only available on Windows (x64) for now."

**Send To (Windows).** Settings -> Agent -> "OS Integration (Send To)" -> Add writes `%APPDATA%\Microsoft\Windows\SendTo\MD-Memo (OCR).lnk` (through a helper script `create_sendto_shortcut.ps1` under `%LOCALAPPDATA%\md-memo\scripts\`); its target is the MD-Memo executable that was running at that moment, with the argument `ocr`, so Explorer runs `md-memo ocr <imagePath>` (section 1.5): no window opens and no process stays. If the executable is later moved to another folder, press Remove and Add again (inferred from the code, `(unverified)` on a real move). Remove deletes the `.lnk`. Right-click an image -> Send to -> MD-Memo (OCR) appends its text to today's note.
...
| `<scrapDir>/assets/` | files | Written by Screen Capture (`YYYY-MM-DD-HHMMSS-cap<N>.png`) and by the hot folder (each processed file is MOVED here as `<YYYY-MM-DD-HHMMSS>-<original name>`, never deleted). Section 3.7. With Git sync on, `git add .` picks these up too. |
...
| `<inbox dir>` (`inbox.dir`; empty = `~/Documents/md-memo/inbox`) | folder | The watched hot folder (section 4.10), created when the hot folder is switched on. Whatever is placed there (images, audio) is moved out and processed, so do not put files there without the user's consent. |
| `<UserCacheDir>/md-memo/components/` (Windows `%LOCALAPPDATA%\md-memo\components`) | Whisper program, models, `<id>.part` partial downloads | On-device Whisper (section 4.10), Windows x64 only. Filled only by the Download buttons in Settings. |
| `%APPDATA%\Microsoft\Windows\SendTo\MD-Memo (OCR).lnk`; `%LOCALAPPDATA%\md-memo\scripts\create_sendto_shortcut.ps1` | shortcut; helper script | The Send To entry (section 4.10), Windows only. Made and removed by the Settings buttons Add and Remove. |
...
| Outbound LLM calls | text (including Ask AI and Quick Capture's AI Send), ghost text, vision, voice, image generation, command-bar AI-mode generation; the same vision call carries images from the hot folder, Screen Capture, Send To and `md-memo ocr` (unless `vision.ocrMode` is `on-device`), and the same voice call carries audio from the hot folder and the Discord bridge (unless `voice.engine` is `whisper-local`) | API key from `config.json` (Gemini keys travel in the URL query for `generateContent`, in `x-goog-api-key` for the Interactions API; OpenAI-style in `Authorization: Bearer`) | per request (120 s general, 30 s ghost text) |
...
| Whisper downloads (Windows x64) | `https://github.com/ggml-org/whisper.cpp/releases/download/b5130/whisper-bin-x64.zip` (program) and a model from `https://huggingface.co/ggerganov/whisper.cpp/...` or `https://huggingface.co/kotoba-tech/kotoba-whisper-v2.0-ggml/...`, or the user's own `voice.whisper.customUrl` (https) | none; SHA-256 checked (pinned for the catalog, optional `customSha256` for a custom URL) | only while the user's Download click runs (resumable, Cancel stops it); never automatic (section 4.10) |
...
### 6.4 Discord Bridge (`pkg/discordbridge`, `app_discordbridge.go`)

An opt-in, poll-only alternative to Mobile Drop for capture that must survive the app being closed: instead of a LAN server the phone connects to, one paired Discord account DMs the user's own bot, and the Go backend polls that DM channel over plain outbound HTTPS (`GET /channels/{id}/messages?after=...`, Discord API v10) - no Gateway/WebSocket connection, no inbound port, no relay or hosting of any kind. Discord itself (not MD-Memo) requires the bot to share at least one server with the paired user before it will let the bot open a DM (otherwise `EnsureDMChannel`/message send fails, error 50007 "Cannot send messages to this user") - a one-time OAuth2 `bot` scope invite into any server the user owns, with no slash command and no ongoing presence there.

- Config: `discordBridge.{enabled, botToken, allowedUserId, pollIntervalSeconds}` (see `setup-guide.md` (b)). `InitDiscordBridge` (re)starts or stops the poller; called at startup and from `SaveConfig` only when these fields actually changed (same guard as `InitScrapEngine`/`InitJevEngine`).
- Connect: on start, verifies the token (`GET /users/@me`) and opens/reuses the bot's DM channel with `allowedUserId` (`POST /users/@me/channels`); retries forever with exponential backoff (5s, doubling, capped at 5 min) on failure, reporting each attempt through `window.onDiscordBridgeStatus({status, message})` (`connecting` | `connected` | `error`) - it never gives up, since the user may fix a typo'd token while the app keeps running. `EnsureDMChannel` rejects an `allowedUserId` that is not all-digits/15-25 chars (`isSnowflake`) before any request is sent, since pasting the visible Discord username instead of the numeric id (Copy User ID) is the most common setup mistake and Discord's own API answers a malformed `recipient_id` with a generic `400`.
- First run baselines to the current latest message id (`limit=1`, no `after`) without processing it, so enabling the bridge never replays the whole DM history; the cursor then persists to `<configDir>/md-memo/discord_bridge_state.json` (NOT inside `scraps.scrapDir`, so it is never seen by Git sync) so a message sent while the app was closed is still picked up on the next launch.
- Every message author is checked against `allowedUserId`; anything else is silently dropped (defence in depth - the DM channel is 1:1 by construction, but this is the sole authorization check).
- Each message's text (if any) and each attachment become one `## Discord [HH:MM:SS]` scrap section apiece (a caption + a photo is two sections), through the exact same `mobileDropItemBody`/`mobileDropKeepFile` pipeline Mobile Drop uses (OCR via `vision.*`, transcription via `voice.*`, same `ErrNotConfigured`/fallback-to-`assets/`-file behaviour); appended to `scraps/YYYY-MM-DD.md` via `pkg/scrap.AppendRaw` (no CLI code-fence wrapping), then `TriggerGitSync()`. Attachment downloads are capped at 20 MB (`discordbridge.DefaultMaxAttachmentBytes`, matching Mobile Drop's single-upload limit) and fetched with no `Authorization` header (Discord's CDN attachment URLs are pre-signed).
- Delivery is reported via `window.onDiscordBridgeMessage({filePath, fileName, timestamp})`, deliberately separate from `onScrapAppended` (the CLI-pipe event): it only refreshes an already-open tab for that file and shows a toast, and never switches the active tab or creates one - a message can arrive at any moment in the background and must not steal focus from whatever the user is editing.
- `TestDiscordBridgeConnection(botToken, allowedUserId)` (Settings -> Sync -> "Mobile capture via Discord" -> Test Connection) runs the same token+channel check synchronously with its own 10s timeout, independent of the running poller.
...
`window.backend.*` (defined in `window_windows.go` / `window_darwin.go`) wraps Go methods bound as `window.backend_<name>`; results arrive through globals such as `window.__onLLMResult`, `__onAutocompleteResult`, `__onSlotAgentResult`, `__onVoiceResult`, `__onMobileDrop*`, `__onCliFilterResult`, `onGitSyncStatus`, `onScrapAppended`, `onDiscordBridgeStatus`, `onDiscordBridgeMessage`. `window.__mdMemoRPC` (section 2) is the only bridge object meant for tooling. Other page globals reachable through `ui eval`: `SlotAgent`, `AutoSelector`, `SlotSnippets`, `JevAction`, `TaskManager`, `VoiceInput`, `FileAnchor`, `ChromeLayout`, `ConfigPack`, `HtmlToMd`, `MdMemoBridge` (its `getConfig()` returns the live config including API keys), `__testHelper`. Do not use them for normal work.

---

## Source of truth

- `main.go` (dispatch, pipe, single instance, UI server, `/api/image`), `app.go`, `app_cli.go`, `cli_ai.go`, `app_rpc.go`, `app_config.go`, `app_scrap.go`, `app_files.go`, `app_inputs.go`, `app_slot.go`, `app_jev.go`, `app_llm.go`, `app_mobiledrop.go`, `app_discordbridge.go`, `app_pack.go`, `ollama_ops*.go`, `window_windows.go`, `window_darwin.go`, `hotkey_darwin.go`, `console_windows.go`
- `pkg/cli/{client,headless,format}.go`, `pkg/ipc/{ipc,rpc_types}.go`, `pkg/appdir`, `pkg/scrap`, `pkg/search`, `pkg/gitsync`, `pkg/configpack/*`, `pkg/slotagent/*` (`mention.go` for `@agent` resolution), `pkg/jev/*` (`guard*.go`, `verifier.go`, `runner.go`, `agent_bridge.go`, `jev_client.go`, `types.go`), `pkg/llm/{llm,audio,ollama}.go`, `pkg/dropzone/{server,dropzone,tunnel,html}.go`, `pkg/discordbridge/{client,poller,ratelimit,state}.go`, `pkg/hotkey`, `pkg/singleinstance`, `pkg/shellenv`, `pkg/encoding`
- `frontend/index.html`, `frontend/js/{app,slot_agent,auto_selector,slot_snippets,jev_action,task_manager,voice_input,file_anchor,chrome_layout,config_pack,i18n,html_to_md,speech_settings}.js`
- Quick Capture, Screen Capture, hot folder, Whisper, Send To, `ocr`: `quickcapture.go`, `quickcapture_windows.go`, `imgcap.go`, `imgcap_windows.go`, `sendto_windows.go`, `app_inbox.go`, `app_speech.go`, `pkg/cli/ocr.go`, `pkg/inbox`, `pkg/ocr`, `pkg/speech`, `pkg/components`
