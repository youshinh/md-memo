...
| `missing` | the agent's program is not found on PATH (`exec.LookPath` through the 30 s cache, and once more without it when the cache says no, so a program installed a moment ago is not refused) | `Agent "hermes" needs "ollama", which was not found in PATH. Install it or choose another agent in agents.yaml.` |
