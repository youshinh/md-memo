...
Substring filter (case-insensitive) over each entry's title and description; Up/Down/Enter/Esc. Entries: New Tab, Open File, Open Folder, "Ask AI", "Command Bar" (opens in the mode used last), "Command Bar: Run a Command" (manual CLI mode), "Command Bar: AI Writes the Command" (AI mode), "Insert task snippet" (ja 「タスクのひな形を挿入」: the snippet list of 3.1.2 at the caret), Mobile Drop, Voice Input, "Quick Capture" (Windows only: it exists only where the backend offers `openQuickCapture`; ja クイックキャプチャ), "Open inbox folder" (ja 「受信フォルダを開く」; only while `inbox.enabled` is true, both platforms), three prompt presets (Polish, Bullet points, Action items: they open the Ask AI bar with the instruction pre-filled), "Diagram: Convert Selection to Mermaid", "Diagram: Generate Image with Gemini", "Diagram: Generate Image Prompt from Mermaid", AI proofread, Export plain text, Zen mode, Full screen, Toggle split (21 commands on macOS with the hot folder off; Windows adds Quick Capture, and the hot folder adds Open inbox folder); plus every note found in the open workspace folder (title, relative path and first line; Enter opens it in a new tab). It has no settings entries.
