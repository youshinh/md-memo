...
Before any run, whatever started it, an agent that acts without asking for permission is confirmed first and nothing in the note changes until the user says Run (3.1.3).
...
Confirmation before a risky agent runs. Every run goes through it: a classic slot, a `{{ @agent }}` task, a slot bound to a profile, a recipe or a resumed approval gate, the Auto selector's agent tasks, and a slot a Quick Actions card inserts. `ParseSlotsRPC` names the agent the run would start (`runAgentKey`, `runAgent`; `slotagent.RunAgentFor`: the `@agent`; else the notation's profile agent; else `default_agent`; an unknown agent or one without a command falls back to `default_agent`, then to the built-in `claude-code`; a recipe and a resumed gate use `default_agent`; an `@agent` is never swapped). A definition is risky when:
