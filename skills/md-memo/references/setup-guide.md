...
Basis: app 1.9.0, source of 2026-09-21 (commit a85494c or later); the keys and checklist rows for the hot folder (`inbox.*`), `vision.ocrMode`, `voice.engine`, `voice.whisper.*`, `shortcuts.quickCapture` and `md-memo ocr` come from the source of 2026-09-24. `(unverified)` = not checked in this repository's source (OS behaviour or an external tool). Read `interfaces.md` first for what each surface does.
