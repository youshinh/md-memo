...
Basis: app 1.10.0, source of 2026-09-21 (commit a85494c or later); the keys and checklist rows for the hot folder (`inbox.*`), `vision.ocrMode`, `voice.engine`, `voice.whisper.*`, `shortcuts.quickCapture` and `md-memo ocr` come from the source of 2026-09-24. `(unverified)` = not checked in this repository's source (OS behaviour or an external tool). Read `interfaces.md` first for what each surface does.
...
5. Never run `md-memo buffer set` (or `replace`) without explicit content and, when the user may be typing, `--expected-hash`; use only tab ids returned by `tab list` (an unknown one is refused with `no such tab`). RPC writes mark the tab dirty and, with autosave, rewrite the file. Never run `md-memo buffer save --overwrite` without the user's yes, and never write a file with `buffer save --as` they did not ask for. A program that talks to the JSON-RPC port itself must send the `token` of `ipc-session.json` as `auth` (writes are refused without it, `interfaces.md` 2.1) and must not print it.
