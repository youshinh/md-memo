...
| Photo/voice item shows `[Mobile Drop: <name>の処理に失敗しました: ...]` (or, in the working tree, is kept under `assets/`) | OCR/transcription failed for that item (key, model, network); the rest of the batch still arrived. | Fix `vision.*` / `voice.*`; resend. |
